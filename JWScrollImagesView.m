...
-(void)addImageView{
    ...
    if (_imageArray.count == 1) {
        [defaultView setImageWithURL:[NSURL URLWithString:[_imageArray objectAtIndex:0]] placeholderImage:[UIImage imageNamed:@"bg_img.jpg"]];
    }else{
        //无图片时
        defaultView.image = [UIImage imageNamed:@"bg_img.jpg"];
    }
...
- (void)setInfoByCurrentImageIndex:(NSUInteger)currentImageIndex {
    ...
    [_imgVCenter setImageWithURL:[NSURL URLWithString:[_imageArray objectAtIndex:currentImageIndex]] placeholderImage:[UIImage imageNamed:@"bg_img.jpg"]];
    
    [_imgVRight setImageWithURL:[NSURL URLWithString:[_imageArray objectAtIndex:(unsigned long)((_currentImageIndex + 1) % _imageArray.count)]] placeholderImage:[UIImage imageNamed:@"bg_img.jpg"]];
    
    [_imgVLeft setImageWithURL:[NSURL URLWithString:[_imageArray objectAtIndex:(unsigned long)((_currentImageIndex - 1 + _imageArray.count) % _imageArray.count)]] placeholderImage:[UIImage imageNamed:@"bg_img.jpg"]];
